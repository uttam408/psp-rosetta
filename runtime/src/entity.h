...
struct Entity {
    real x, y;
    real Hspeed, Vspeed;   /* backing fields; use the accessors below       */
    real direction;        /* degrees; kept in sync by the h/vspeed setters */
    real gravity, friction;
    real _moveX, _moveY;   /* sub-pixel accumulators for moveBy              */
...
real ent_hspeed(const Entity *e);
real ent_vspeed(const Entity *e);
void   ent_set_hspeed(Entity *e, real v);
void   ent_set_vspeed(Entity *e, real v);
real ent_speed(const Entity *e);
