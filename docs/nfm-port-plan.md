...
| M3 | `Medium` + `Plane`: camera, sort, per-poly lighting, fog, sky/ground; stage loader (`set`/`chk`/`fix`, then `pile`/mountains/walls) | **Core done, not compared to the original yet.** `runtime/nfm/medium.c` ports `Plane.d`, `ContO.d` and `Medium.d` (sky and ground bands) in the original 800x450 integer space, scaled by 0.6 only at fill time. `stage.c` loads `.pstg`, replays the environment directives and places `set`/`chk`/`fix` pieces, `pile` (procedural rock mounds, `pile.c`, seeded by a `java.util.Random` port verified against the JDK) and the `maxr/l/t/b` walls (`thewall` every 4800 units). `nfm_view <pak> data/stage/1 --shot out.bmp` renders the intro stage (125 pieces, 845 polys, 0.27 ms/frame on the host; stage 10: 390 pieces, 0.33 ms; no directive skipped). **Still missing:** mountains, clouds, stars, shadows, damage/chip effects, road markings for `flx` polys |
| M4 | `Mad` + `Wheels` + `Trackers`: physics and collision, car drives | **Done on the desktop build (not yet diffed against the original).** `Wheels.make` is ported (`wheels.c`: 19 planes per wheel, spin/steer, LOD). Stock car stats come from `spec/gen_nfm_cars.py` -> `gen/cardefs.h`. `Mad.drive/reseto/regx/regy/regz/colide` are a mechanical translation of `Mad.java` (`spec/gen_nfm_mad.py` -> `gen/mad_gen.inc`, wrapped by `mad.c`/`mad.h`) that keeps Java's operand order and float/int typing; cosmetics (sound, dust, sparks, HSB tinting, replay) are no-op hooks and `Math.random` is a per-car LCG. `Trackers` are built from the pieces' `PmTrack` rows plus the `maxr/l/t/b` slabs and split into 3000-unit sectors (`trackers_divide`); checkpoints and fix points come from the stage. `nfm_view <pak> data/stage/1 --drive audir8 300 --shot out.bmp` drives stage 1: accelerates, takes the ramp, clears checkpoint 1, hits the end wall and recovers. **Missing:** pile trackers, damage vertices are tracked in `CarObj` but not drawn, car-car `colide` is untested, keyboard driving in the viewer, PSP hookup |
